...
REGION=europe-west1
IMAGE=$REGION-docker.pkg.dev/$PID/tf/fleet:v3

echo "== 1. imagen =="
gcloud builds submit --project $PID --tag $IMAGE --quiet > /tmp/b.log 2>&1 \
  && echo "   OK $IMAGE" || { echo "   FALLO build"; tail -12 /tmp/b.log; exit 1; }

echo "== 2. gateway =="
gcloud run deploy tf-gateway --project $PID --image $IMAGE --region $REGION \
...
for A in ingestor linker auditor impact; do
  gcloud run deploy tf-agent-$A --project $PID --image $IMAGE --region $REGION \
